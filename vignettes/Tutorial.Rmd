...
### Some Notes
Using `plot.gaussmodel`, we can plot cumulative distribution functions
by `cum=TRUE`:

```{r, eval=TRUE, out.height="80%", out.width="70%"}
plot(gm1, cum=TRUE, nmax=1)
```
...
### Some Notes
Using `plot.expmodel`, we can plot cumulative distribution functions
by `cum=TRUE`:
